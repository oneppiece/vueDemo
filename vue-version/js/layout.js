...
var Layout = function() {
	...
	var handleSidebarAndContentHeight = function() {
		...
		if (body.hasClass("page-footer-fixed") === true
			...
			if (body.hasClass('page-sidebar-fixed')) {
				...
				if ((height + headerHeight + footerHeight) <= Metronic
						.getViewPort().height) {
					height = Metronic.getViewPort().height - headerHeight
							- footerHeight;
				}
	...
	var handleSidebarMenuActiveLink = function(mode, el) {
		...
		if (mode === 'click' || mode === 'set') {
			...
			menu.find("li > a").each(
					function() {
						...
						// url match condition
						if (path.length > 1
								&& url.substr(1, path.length - 1) == path
										.substr(1)) {
							el = $(this);
							return;
						}
		...
		if (el.attr('href').toLowerCase() === 'javascript:;'
		...
		if (mode === 'click') {
			if (Metronic.getViewPort().width < resBreakpointMd
					&& $('.page-sidebar').hasClass("in")) { // close the menu on
				// mobile view while
				// laoding a page
	...
	var handleSidebarMenu = function() {
		...
		$('.page-sidebar')
				.on(
						...
						function(e) {
							...
							if (Metronic.getViewPort().width >= resBreakpointMd
									&& !$('.page-sidebar-menu').attr(
											"data-initialized")
									&& $('body')
											.hasClass('page-sidebar-closed')
							...
							if (hasSubMenu === false) {
								if (Metronic.getViewPort().width < resBreakpointMd
										&& $('.page-sidebar').hasClass("in")) { // close
									// the
									// menu
									// on
									// mobile
							...
							if (keepExpand !== true) {
								parent.children('li.open').children('a')
										.children('.arrow').removeClass('open');
								parent.children('li.open').children(
										'.sub-menu:not(.always-open)').slideUp(
										slideSpeed);
							...
							$
									.ajax({
										...
										success : function(res) {
											...
											Layout.fixContentHeight(); // fix
											// content
											// height
											Metronic.initAjax(); // initialize
											// core
											// stuff
										},
		...
		// handle scrolling to top on responsive menu toggler click when header
		// is fixed for mobile view
		$(document).on('click',
		...
		$('.page-header').on(
				...
				function(e) {
					...
					$('.page-header .search-form .form-control').on(
							'blur',
							function(e) {
								$(this).closest('.search-form').removeClass(
										"open");
								$(this).unbind("blur");
							});
				});
	...
	var handle100HeightContent = function() {
		...
		if (target.hasClass('portlet')) {
			...
			if (Metronic.getViewPort().width < resBreakpointMd) {
				Metronic.destroySlimScroll(portletBody
						.find('.full-height-content-body')); // destroy
				// slimscroll
				return;
			}
			...
			height = height
					...
					- parseInt(target.find('.portlet-body').css(
							'padding-bottom')) - 2;
			...
			if (Metronic.getViewPort().width < resBreakpointMd) {
				Metronic.destroySlimScroll(target
						.find('.full-height-content-body')); // destroy
				// slimscroll
				return;
			}
	...
	return {
		...
		initSidebar : function() {
			...
			if (Metronic.isAngularJsApp()) {
				handleSidebarMenuActiveLink('match'); // init sidebar active
				// links
			}
